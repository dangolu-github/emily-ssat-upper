(() => {
  ...
  const storageKey='emily-ssat-c01-hw01-v1',assignmentId='EMI-C01-HW01';
  ...
  function updateCount(){count.textContent=Object.keys(answers()).length+' of 22 answered';}
  ...
  function clean(a){const x={};for(let n=1;n<=22;n++)if(/^[A-E]$/.test(a['q'+n]||''))x['q'+n]=a['q'+n];return x;}
  ...
  submit.addEventListener('click',async()=>{
    ...
    if(Object.keys(answers()).length!==22){say('Answer all 22 questions before submitting.');for(let n=1;n<=22;n++)if(!answers()['q'+n]){form.querySelector('input[name="q'+n+'"]').focus();break;}return;}
  ...
  document.getElementById('download-answers').addEventListener('click',()=>{
    const a=answers(),lines=['Emily | SSAT Upper | Class 01 homework','Analogies and Reading',''];for(let n=1;n<=22;n++)lines.push(n+'. '+(a['q'+n]||'[unanswered]'));
    const url=URL.createObjectURL(new Blob([lines.join('\n')+'\n'],{type:'text/plain;charset=utf-8'})),link=document.createElement('a');link.href=url;link.download='Emily-Class01-Homework-Answers.txt';link.click();setTimeout(()=>URL.revokeObjectURL(url),1000);
  });
