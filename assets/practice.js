(() => {
  ...
  async function load(){
    ...
    try{const data=await EmilyAPI.call('practiceCatalog');if(generation!==epoch||!EmilyAPI.signedIn())return;root.replaceChildren();
      data.sets.forEach(set=>{const card=el('section','');card.className='mock-set';card.append(el('p',set.collectionLabel+' · '+set.levelLabel),el('h2',set.title));const sections=el('div','');sections.className='mock-sections';
